# Originally VectorQuantize.py by Brian McFee <brm2132@columbia.edu>
# Slightly modified for Python 3
...
class VectorQuantizer(BaseEstimator, TransformerMixin):
    def __init__(self, clusterer: BaseEstimator = None,
                 n_atoms: int = 32, sparse: bool = True,
                 batch_size = 1024, n_quantizers = 1):
        ...
        if clusterer is None:
            self.clusterer = cluster.MiniBatchKMeans(n_clusters=n_atoms)
        else:
    ...
    def transform(self, X: np.array):
        ...
        if self.sparse:
            X_new = sparse.lil_matrix((n, C.shape[0]))
        else:
